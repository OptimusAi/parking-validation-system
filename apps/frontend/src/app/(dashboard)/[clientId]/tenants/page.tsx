...
export default function TenantsPage() {
  ...
  const [form, setForm] = useState({ name: '', primaryColor: '#1B4F8A', accentColor: '#2E86C1' });
  ...
  const createMutation = useMutation({
    mutationFn: () => mockApi.createTenant(form),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tenants'] });
      setDialogOpen(false);
      setForm({ name: '', primaryColor: '#1B4F8A', accentColor: '#2E86C1' });
      setLogoFile(null);
      setLogoPreview(null);
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Button
          ...
          onClick={() => router.push(`/${clientId}/${row.id}/dashboard`)}
        >
          Open
        </Button>
      ),
    ...
    <Box>
      ...
      <Paper sx={{ overflow: 'hidden' }}>
        <DataGrid
          ...
          onRowClick={(params) => router.push(`/${clientId}/${params.row.id}/dashboard`)}
          sx={{
            border: 'none',
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            ...
            {/* Logo Upload */}
            <Box>
              <Typography variant="body2" fontWeight={500} gutterBottom>Logo (optional)</Typography>
              <Box
                onDrop={handleDrop}
            ...
            <Stack direction="row" spacing={2}>
              <Box>
                <Typography variant="body2" fontWeight={500} gutterBottom>Primary Color</Typography>
                <TextField type="color" value={form.primaryColor} onChange={(e) => setForm((f) => ({ ...f, primaryColor: e.target.value }))} sx={{ width: 100 }} />
              </Box>
              <Box>
                <Typography variant="body2" fontWeight={500} gutterBottom>Accent Color</Typography>
            ...
            <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, overflow: 'hidden' }}>
              <Box sx={{ p: 1.5, bgcolor: form.primaryColor, color: 'white', textAlign: 'center' }}>
                <Typography variant="body2" fontWeight={700}>{form.name || 'Tenant Name'}</Typography>
              </Box>
